...
def infer_tcutoff(args, times, derSampledTimes, ancSampledTimes, ancientGLs, ancientHapGLs):
	observed_times = []
	if np.size(times) > 0:
		valid_times = np.array(times).ravel()
		valid_times = valid_times[np.isfinite(valid_times)]
		valid_times = valid_times[valid_times >= 0]
		if len(valid_times) > 0:
			observed_times.append(np.max(valid_times))
	for arr in (derSampledTimes, ancSampledTimes):
		if np.size(arr) > 0:
			valid = np.array(arr).ravel()
			valid = valid[np.isfinite(valid)]
			valid = valid[valid >= 0]
			if len(valid) > 0:
				observed_times.append(np.max(valid))
	for ancient in (ancientGLs, ancientHapGLs):
		if np.size(ancient) > 0:
			valid = np.array(ancient)[:,0]
			valid = valid[np.isfinite(valid)]
			valid = valid[valid >= 0]
			if len(valid) > 0:
				observed_times.append(np.max(valid))
	if args.timeBins != None:
		try:
			observed_times.extend([float(x) for x in args.timeBins])
		except ValueError:
			pass
	if len(observed_times) == 0:
		return 1.0
	return max(1.0, float(np.ceil(max(observed_times))))
...
def load_data(args):
	...
	if tCutoff == None or tCutoff <= 0:
		tCutoff = infer_tcutoff(args, times, derSampledTimes, ancSampledTimes, ancientGLs, ancientHapGLs)
		args.tCutoff = tCutoff

	epochs = np.arange(0.0,tCutoff,int(1))
	# loading population size trajectory
	if args.coal != None:
		coal_epochs = np.atleast_1d(np.genfromtxt(args.coal, skip_header=1, skip_footer=1, dtype=float)).ravel()
		coal_rates = np.atleast_1d(np.genfromtxt(args.coal, skip_header=2, dtype=float)).ravel()
		coal_epochs = coal_epochs[np.isfinite(coal_epochs)]
		coal_rates = coal_rates[np.isfinite(coal_rates)]
		if len(coal_epochs) == 0 or len(coal_rates) < 4:
			raise ValueError(f"Failed to parse a valid Relate .coal trajectory from {args.coal}.")
		N = 1/coal_rates[2:-1]
		if len(N) == 0:
			raise ValueError(f"Parsed an empty effective population size trajectory from {args.coal}.")
		N = np.array(list(N)+[N[-1]])
		bin_index = np.digitize(epochs, coal_epochs) - 1
