...
if __name__ == "__main__":
    ...
    root_children = np.where(np.array(List[1]) == "root")[0]
    if len(root_children) < 2:
        raise ValueError("Failed to identify two root children in RelateSamples Newick tree. Please check the .newick format produced by SampleBranchLengths.")
    child1index = root_children[0]
    child2index = root_children[1]
    CalculateDerivedBelow(List,  List[0][child1index], child1index)
    CalculateDerivedBelow(List,  List[0][child2index], child2index)
